...
from myproject.models import Experiment, DoseResponse, JagsSampling, SensitivityScore, CellLine, MutExpMetadata,Drug,Gene,GeneExpression,Mutation
...
def get_experiments(cell_line,drug):
    data = db.session.query(Experiment, JagsSampling, SensitivityScore) \
        ...
        .join(SensitivityScore, SensitivityScore.exp_id == Experiment.id).filter(Experiment.cellosaurus_id == cell_line, Experiment.standard_drug_name == drug)#.all()
    ...
    df = df[['cellosaurus_id', 'standard_drug_name','dataset','info','n_dosage','min_dosage','max_dosage',
             'dose_response','ic50_mode','ic90_calculate','ec50_calculate','einf_calculate','auc_calculate','fitted_mae']]
    # print(df)
...
@api_blueprint.route('/biomarker/<string:exp_mut>/<string:cancer_type>/gene/<string:gene>/<string:pos_neg>/<string:n_top>', methods=['GET'])
def get_gene_express_mut(exp_mut,cancer_type,gene,pos_neg,n_top):
    if exp_mut=='exp':
        express_data = db.session.query(GeneExpression).filter(GeneExpression.cancer_type == cancer_type, GeneExpression.gene == gene, GeneExpression.dataset == 'All')#.all()
        ...
        mutation_data = db.session.query(Mutation).filter(Mutation.cancer_type == cancer_type, Mutation.gene == gene, Mutation.dataset == 'All')#.all()
...
@api_blueprint.route('/biomarker/<string:exp_mut>/<string:cancer_type>/drug/<string:drug>/<string:pos_neg>/<string:n_top>', methods=['GET'])
def get_drug_express_mut(exp_mut,cancer_type,drug,pos_neg,n_top):
    if exp_mut=='exp':
        express_data = db.session.query(GeneExpression).filter(GeneExpression.cancer_type == cancer_type, GeneExpression.standard_drug_name == drug, GeneExpression.dataset == 'All')#.all()
        df = pd.read_sql(express_data.statement, db.session.bind)
        df = df[['standard_drug_name','gene','dataset','cancer_type','correlation','pvalue','n_cell_line']]
        if pos_neg == 'pos':
            df = df[df['correlation']>=0].sort_values(by='correlation', ascending=False).head(int(n_top))
        ...
        mutation_data = db.session.query(Mutation).filter(Mutation.cancer_type == cancer_type, Mutation.standard_drug_name == drug, Mutation.dataset == 'All')#.all()
        df = pd.read_sql(mutation_data.statement, db.session.bind)
        df = df.rename(columns={'statistic':'effect_size', 'provided_statistic':'provided_effect_size'})
        df = df[['standard_drug_name','gene','dataset','cancer_type','effect_size','pvalue','n_mut','n_wt']]
...
@api_blueprint.route('/biomarker/<string:exp_mut>/<string:cancer_type>/info/<string:gene>/<string:drug>', methods=['GET'])
def get_drug_gene_express_mut(exp_mut,cancer_type,gene,drug):
    if exp_mut=='exp':
        express_data = db.session.query(GeneExpression).filter(GeneExpression.gene==gene, GeneExpression.standard_drug_name == drug, GeneExpression.dataset == 'All', GeneExpression.cancer_type == cancer_type)#.all()
        df = pd.read_sql(express_data.statement, db.session.bind)
        df = df[['standard_drug_name','gene','dataset','cancer_type','correlation','pvalue','n_cell_line']]
        ...
        mutation_data = db.session.query(Mutation).filter(Mutation.gene==gene, Mutation.standard_drug_name == drug, Mutation.dataset == 'All', Mutation.cancer_type == cancer_type)#.all()
        df = pd.read_sql(mutation_data.statement, db.session.bind)
        df = df.rename(columns={'statistic':'effect_size', 'provided_statistic':'provided_effect_size'})
        df = df[['standard_drug_name','gene','dataset','cancer_type','effect_size','pvalue','n_mut','n_wt']]
...
def get_cl_mutation(cell_line):
    cell_line_records = db.session.query(CellLine.cellosaurus_index).filter(CellLine.cellosaurus_id==cell_line).distinct()
    cell_line_list = [c.cellosaurus_index for c in cell_line_records]
    print(cell_line_list)

    data = db.session.query(MutExpMetadata).filter(MutExpMetadata.cellosaurus_index==cell_line_list[0],MutExpMetadata.score=='mutation')#.all()
    df = pd.read_sql(data.statement, db.session.bind)
    df = df[['gene','values']]
...
def get_cl_express(cell_line):
    ...
    data = db.session.query(MutExpMetadata).filter(MutExpMetadata.cellosaurus_index==cell_line_list[0],MutExpMetadata.score=='gene_expression')#.all()
    df = pd.read_sql(data.statement, db.session.bind)
    df = df[['gene','values']]
